...
fn main(_image: Handle, system_table: SystemTable<Boot>) -> Status {
    let (boot_info, kernel_entry, kernel_stack) = {
        ...
        let rtc = load_file("rtc.elf");
        let ps2_kbd = load_file("ps2_kbd.elf");
        ...
        if let Ok(handle) = boot_services.get_handle_for_protocol::<MpServices>() {
            ...
            let bsp = mp.who_am_i().unwrap();
            let count = mp.get_number_of_processors().unwrap();
            for i in 0..count.total {
                let processor = mp.get_processor_info(i).unwrap();
                if i != bsp && processor.is_enabled() && cpu_count < apic_ids.len() {
                    apic_ids[cpu_count] = processor.processor_id as u32;
                    cpu_count += 1;
                }
            }
        }
        ...
        let info = BootInfo {
            ...
            stride: mode.stride(),
            programs: [app, app2, clock, dzen_clock, ping, pong, rtc, ps2_kbd],
            heap_ptr,
            heap_len,
            ap_trampoline,
            cpu_count,
